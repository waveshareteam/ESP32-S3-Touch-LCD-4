...
Arduino sketches and bundled libraries are under [arduino](arduino/). `03_Drawing_points` and `09_LVGL_Widgets` probe GT911 at runtime and continue without pointer input on ESP32-S3-LCD-4. The current `Build Examples` workflow compiles first-party product sketches with Arduino ESP32 core `3.3.11` and the libraries already stored in [arduino/libraries](arduino/libraries/). When using Arduino examples locally, keep the board options aligned with CI: ESP32-S3 Dev Module, 16 MB Flash, OPI PSRAM, USB CDC on boot, and the `app3M_fat9M_16MB` partition scheme.
